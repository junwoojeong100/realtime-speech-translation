"""
YouTube 스트리밍 한글 자막 생성기 (LLM Speech, azure-ai-transcription SDK)

yt-dlp → ffmpeg 파이프라인으로 오디오를 다운로드 완료 없이 스트리밍 수신하고,
10초 단위 청크로 LLM Speech 모델을 azure-ai-transcription SDK로 호출해
결과가 오는 대로 즉시 출력합니다.
(REST API 직접 호출 구현은 translate_llmspeech_rest.py 참고)
...
사용법:
  ...
  python translate_llmspeech_sdk.py "https://www.youtube.com/watch?v=VIDEO_ID" --chunk 10
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="YouTube 동영상 준실시간 한글 자막 생성 (LLM Speech)",
    )
