...
| `text_translate.py` | 텍스트 번역 | Azure AI Translator(NMT) **또는** Azure OpenAI(LLM) | NMT / `gpt-5.4-mini` |
...
| `AZURE_OPENAI_ENDPOINT` | (`--engine llm`) Azure OpenAI 엔드포인트 (미설정 시 `AZURE_SPEECH_RESOURCE_ID`로 유추) |
| `AZURE_OPENAI_DEPLOYMENT` | (`--engine llm`) 모델 배포 이름 (기본 `gpt-5.4-mini`) |
| `AZURE_OPENAI_API_VERSION` | (`--engine llm`) API 버전 (기본 `2024-10-21`) |
...
### text_translate.py — 외신 텍스트 번역 (NMT / LLM)

로이터·AP·CNN 등 외신 영어 텍스트를 한국어로 번역합니다. **두 가지 엔진**을 `--engine`으로 선택합니다.

- **`nmt`(기본)** — Azure AI Translator: 저비용·고속·예측가능 → **대량/실시간 인입**에 적합
- **`llm`** — Azure OpenAI(기본 `gpt-5.4-mini`) 직접 호출: **문맥·뉘앙스·말투** 품질↑ → 사설·인터뷰 등 **고가치 기사**에 적합
...
> **NMT vs LLM 선택 가이드**: 관용구·맥락이 중요한 문장에서 LLM이 더 정확합니다. 예) "the Fed's *pivot*" → NMT는 "회귀"로 오역, LLM은 "방향 전환"으로 정확. "*whipsawed*" → NMT "급락", LLM "급등락". 대량·실시간은 **NMT 기본**, 고가치 기사만 **LLM 선택 적용**하는 **하이브리드**가 비용·품질 균형에 유리합니다. (`--engine llm`은 Azure OpenAI 모델 배포 필요)
...
> LLM 직접 번역(Azure OpenAI)은 이미 `text_translate.py --engine llm`로 제공됩니다. 아래는 그 외 대안입니다.
...
| **Translator LLM 번역** (GPT 기반 옵션) | Translator API에서 LLM 모델로 문맥·톤 조절 번역 | NMT·직접 호출 대신 **Translator 한 곳에서** LLM 품질이 필요할 때 |
